...
def read_bag_file(filename):
    ...
    q_vicon_array = np.zeros((num_vicon_var_msg,4))
    for topic, msg, t in bag.read_messages(topics=[vicon_var_topic]):
        # pdb.set_trace()
        v_time_array[vicon_index,0] = t.to_sec()
        q_vicon_array[vicon_index,:] = np.array([msg.transform.rotation.w, msg.transform.rotation.x, msg.transform.rotation.y, msg.transform.rotation.z])
    ...
    return time_array, xd_array, xd_dot_array, xd_ddot_array, IMU_W_array, \
            ...
            gainR_array, dt_vicon_array, q_v_array, q_imu_array, v_time_array, q_vicon_array
...
def vicon_imu_comaprison(t_v, t_i, vicon_sent, vicon_received, latex):
    ...
    plt.subplot(411)
    plt.plot(t_i[:], vicon_received[:,0],'b', label = 'Received')
    plt.plot(t_v[:], vicon_sent[:,0],'r', label = 'Sent')
    plt.xlabel('Time (sec since epoch) ')
    plt.ylabel('w')
    plt.legend()

    plt.subplot(412)
    plt.plot(t_i[:], vicon_received[:,1],'b', label = 'Received')
    plt.plot(t_v[:], vicon_sent[:,1],'r', label = 'Sent')
    plt.xlabel('Time (sec since epoch) ')
    plt.ylabel('x')

    plt.subplot(413)
    plt.plot(t_i[:], vicon_received[:,2],'b', label = 'Received')
    plt.plot(t_v[:], vicon_sent[:,2],'r', label = 'Sent')
    plt.xlabel('Time (sec since epoch) ')
    plt.ylabel('y')

    plt.subplot(414)
    plt.plot(t_i[:], vicon_received[:,3],'b', label = 'Received')
    plt.plot(t_v[:], vicon_sent[:,3],'r', label = 'Sent')
    plt.xlabel('Time (sec since epoch) ')
    plt.ylabel('z')
    # plt.set_xlim(15, 35)
...
if __name__ == "__main__":
    ...
    try:
        ...
        time_array, xd_array, xd_dot_array, xd_ddot_array, IMU_W_array, IMU_RPY_array, \
            ...
            dt_vicon_array, q_v_array, q_imu_array, v_time_array, q_vicon_array = read_bag_file(args.input_file)
    ...
    vicon_imu_comaprison(v_time_array, time_array, q_vicon_array, q_v_array, latex)
